...
# def UDPSendMessage(clientAddr,IDSend,labelSend,xSend,ySend,rSend,aSend,cSend,bufferSize,clientDataTime):
#     try:
...
#         #Send the ID 
#         # objStr = "{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f};".format(IDSend, xSend, ySend, rSend, aSend, cSend)
#         #Send the label
#         objStr = "{},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f};".format(labelSend, xSend, ySend, rSend, aSend, cSend)
        
#         outMsg += objStr
#         UDPServerSocket.sendto(str.encode(outMsg), clientAddr)
#         print("Sent message to ", clientAddr)
#         clientDataTime = now
def send_ack(s, addr, port,infoToSend):
    # coords = {
    #     "type": 0,
    ...
    #Hacky method to individually parse items to hash, since numpy array is easy to pass in method and hash is serializable by JSON
    # coords = infoToSend


    data = bytes(json.dumps(infoToSend), 'utf-8')
    size = len(data)
...
def detect(opt):
    ...
    counter = 0
    loopTimer = time.time()
    imshowTimer = time.time()

    dat = b''
    dump_buffer(UDPServerSocket)
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        dump_buffer(UDPServerSocket)
        while True:
            seg, addr = UDPServerSocket.recvfrom(MAX_DGRAM)
            if struct.unpack("B", seg[0:1])[0] > 1:
                dat += seg[1:]
            else:
                dat += seg[1:]
                rawImg = cv2.imdecode(np.fromstring(dat, dtype=np.uint8), 1)
                dat = b''
        ...
        img = [letterbox(x, img_size, stride=stride, auto=rect and auto)[0] for x in img0]
